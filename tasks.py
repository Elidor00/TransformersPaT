...
class PARSING(TokenClassificationTask):
    """
    PaT (Parsing as Tagging)
    Parsing: try to predict the relative position of the token with respect to its head
    (i.e. the arc between the token and its head) and the arc label
    ...
    def read_examples_from_file(self, data_dir, mode: Union[Split, str]) -> List[InputExample]:
        ...
        with open(file_path, encoding="utf-8") as f:
            for sentence in parse_incr(f):
                ...
                for token in sentence:
                    words.append(token["form"])
                    element = '0' if token["head"] == 0 else str(token["head"] - token["id"])
                    element = element + token["deprel"]
    ...
    def write_predictions_to_file(self, writer: TextIO, test_input_reader: TextIO, preds_list: List):
        ...
        for sentence in parse_incr(test_input_reader):
            ...
            if len(sentence) == len(s_p):
                ...
                for token in sentence:
                    out += f'{token["form"]} ' \
                           f'({0 if token["head"] == 0 else token["head"] - token["id"] + token["deprel"]}|{s_p.pop(0)})'
                out += "\n"
                writer.write(out)
            else:
                error += 1
            ...
            with open(file_path, encoding="utf-8") as f:
                for sentence in parse_incr(f):
                    for token in sentence:
                        ...
                        element = '0' if token["head"] == 0 else str(token["head"] - token["id"])
                        element = element + token["deprel"]
                        # element = ('0' if token["head"] == 0 else str(token["head"] - token["id"]), token["deprel"])
                        self.labels.add(element)
